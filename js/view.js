...
{
  var id=0;
   var databaseRef = firebase.database().ref('vendornum/');
            var data = {
                vnum:id
            }
            var updates = {};
            updates['/vendornum/' + 0] = data;            // + usn makes it a primary key.
...
function search()
{
  ...
  databaseRef.once('value', function(snapshot) {
            snapshot.forEach(function(childSnapshot) {
                ...
                switch(strfilter)
                {
                  case 'Email': query=childData.email;
                                break;
                  case 'Name': query=childData.name;
                                break;
                  case 'Ticket ID': query=childData.tid;
                                break;
                  case 'Phone': query=childData.phone;
                                break;
                  case 'MOP': query=childData.mop;
                                break;
                }
                alert(query);
